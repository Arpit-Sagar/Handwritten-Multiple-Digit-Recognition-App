...
#button-1 for left mouse button-->b1-motion for mouse movement
...
def Recognize_Digit():
    ...
    for cnt in contours:
        ...
        #return the indices of maximum values
        ...
        if int(max(pred) * 100)<60 :
            print("--Adequate digit not drawn--")
            ans='Not a digit'
            cv2.putText(image,ans, (x, y - 5), font, fontScale, color, thickness)
        else:
            cv2.putText(image, data, (x, y - 5), font, fontScale, color, thickness)
    #new window for showing predicted results
